...
load E:\HOME\+PROJECTS\++++2015_2016_ATCERN\1aOpticalTPC\CF4lightGenerator\S2_PureHe_1bar_GEM_NORM;
...
for i=1:length(Admixture_), YieldInterp(i,:) = LightSpectrumInHeCF4(Admixture_(i), Lambda_); end
...
phEl_TOT_LIP     = [0.0725, 0.085];
Err_phEl_TOT_LIP = [0.075*0.2, 0.085*sqrt(0.17^2+0.10^2)];
conc_TOT_LIP     = [0.4, 1];

%Fraga with CCD camera (2GEMs, hole/kapton/pitch = 80/50/140
NORM                 = (0.6*1e-5)/0.09;
phEl_TOT_LIP_CCD     = [0.15 0.25, 0.3, 0.35, 0.6, 0.8]*1e-5/NORM;
Err_phEl_TOT_LIP_CCD = phEl_TOT_LIP_CCD*0.2; %FIXME: error CCD?
conc_TOT_LIP_CCD     = [0.05, 0.2, 0.3, 0.4, 0.8, 1];

%Note: 80/60/140 can give up to a factor x2 less.


figure; hold on;
plot(Admixture_(Admixture_>=0.4), phEl_UV(Admixture_>=0.4), '-');
plot(Admixture_(Admixture_>=0.4), phEl_vis(Admixture_>=0.4),'-r');
plot(Admixture_(Admixture_>=0.4), phEl_IR(Admixture_>=0.4), '-g');
plot(Admixture_(Admixture_>=0.4), phEl_vis(Admixture_>=0.4)+phEl_IR(Admixture_>=0.4), '-k');
...
plot(Admixture_(Admixture_<=0.4), phEl_UV(Admixture_<=0.4), ':b');
plot(Admixture_(Admixture_<=0.4), phEl_vis(Admixture_<=0.4),':r');
plot(Admixture_(Admixture_<=0.4), phEl_IR(Admixture_<=0.4), ':g');
plot(Admixture_(Admixture_<=0.4), phEl_vis(Admixture_<=0.4)+phEl_IR(Admixture_<=0.4), ':k');
