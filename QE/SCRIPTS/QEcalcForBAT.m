...
dir       = 'E:\HOME_RareEventsGroup\Diego\QE_Calc\QEdata\';
dirYield  = 'E:\HOME_RareEventsGroup\Diego\QE_Calc\CF4lightGenerator\';

lambda_ = 200:1:900;

Yield67Ar_  = LightSpectrumInArCF4_sec(0.33, lambda_);
Yield60Ar_  = LightSpectrumInArCF4_sec(0.4, lambda_);
Yield70Ar_  = LightSpectrumInArCF4_sec(0.3, lambda_);
Yield80Ar_  = LightSpectrumInArCF4_sec(0.2, lambda_);
Yield90Ar_  = LightSpectrumInArCF4_sec(0.1, lambda_);
Yield95Ar_  = LightSpectrumInArCF4_sec(0.05, lambda_);
Yield98Ar_  = LightSpectrumInArCF4_sec(0.02, lambda_);
Yield60He_  = LightSpectrumInHeCF4(0.4, lambda_);

file = 'CF4_scintillation'; %For pure CF4: it might be good to include in the above generator!
...
legend('H7826', 'H7826-01', 'R1926', 'R7378', 'R5070', 'H10426', 'H10426-01', 'NEXT', 'R375', 'CCD');
...
plot(lambda_, Yield67Ar_ .* Transm_ .* QE_5070_,      'b:');
...
plot(lambda_, Yield67Ar_ .* Transm_ .* QE_5070_      .* pi*(2.2/2)^2/(4*pi*10^2), 'b:');
...
figure; title('T*Y*QE (R7378)'); hold on; 
plot(lambda_, Yield80Ar_ .* Transm_ .* QE_5070_ , '-');
plot(lambda_, Yield95Ar_ .* Transm_ .* QE_5070_ , 'r-');
plot(lambda_, Yield60He_ .* Transm_ .* QE_5070_ , 'g-');
plot(lambda_, Yield100CF4_ .* Transm_ .* QE_5070_ , 'k-'); box;
legend('Ar/CF4 80/20', 'Ar/CF4 95/5', 'He/CF4 0/60', 'CF4 100');
